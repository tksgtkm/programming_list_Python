...
class Image:
    ...
    def __init__(self, width=None, height=None, filename=None, background=None, pixels=None):
        """
        コンストラクト関数の一つを用いてイメージを作る
        from_file(), create(), from_data()
        """
    ...
    def load(self, filename):
        """
        filenameというファイルから画像をロードする
        形式はファイルの接尾辞によって決定される
        """
        ...
        if module is not None:
            ...
            raise Error("no Image module can load files of type {}".format(os.path.splitext(filename)[1]))
        
    def save(self, filename=None):
        """
        画像をfilenameという名前のファイルに保存する
        形式はファイルの接尾辞によって決定される
        """
        ...
        if module is not None:
            ...
            raise Error("no Image module can save files of type {}".format(os.path.splitext(filename)[1]))
    ...
    def set_pixel(self, x, y, color):
        """
        指定されたピクセルを指定された色に設定される。
        xとyは必ず入っていなければいけない。
        範囲：colorはARGB int でなければいけない
        """
        ...
        if fill is not None:
            ...
            for y in range(-halfHeight, halfHeight + 1):
                for x in range(-halfWidth, halfWidth + 1):
                    delta_x = x / halfWidth
                    delta_y = y / halfHeight
                    if ((delta_x * delta_x) + (delta_y * delta_y)) <= 1:
                        self.set_pixel(int(round(midX + x)), int(round(midY + y)), fill)
        if outline is not None:
            if x1 > x0:
                midX = ((x1 - x0) // 2) + x0
            else:
            ...
            def ellipse_point(delta_x, delta_y):
                self.set_pixel(midX + delta_x, int(round(midY + delta_y)), outline)
                self.set_pixel(midX - delta_x, int(round(midY - delta_y)), outline)
                self.set_pixel(midX + delta_x, int(round(midY - delta_y)), outline)
                self.set_pixel(midX - delta_x, int(round(midY + delta_y)), outline)
            ...
            delta_y = b
            p = b2 - (a2 * b) + (a2 / 4)
            ellipse_point(delta_x, delta_y)
            while (a2 * (delta_y - 0.5)) > (b2 * (delta_x + 1)):
                if p < 0:
                    p += b2 * ((2 * delta_x) + 3)
                    delta_x += 1
                else:
                    p += (b2 * ((2 * delta_x) + 3)) + (a2 * ((-2 * delta_y) + 2))
                    delta_x += 1
                    delta_y -= 1
                ellipse_point(delta_x, delta_y)
            p = ((b2 * ((delta_x + 0.5) ** 2)) + (a2 * ((delta_y - 1) ** 2)) - (a2 * b2))
            while delta_y > 0:
                if p < 0:
                    p += (b2 * ((2 * delta_x) + 2)) + (a2 * ((-2 * delta_y) + 3))
                    delta_x += 1
                    delta_y -= 1
                else:
                    p += a2 * ((-2 * delta_y) + 3)
    ...
    def subsample(self, stride):
        assert(2 <= stride <= min(self.width // 2, self.height // 2) and isinstance(stride, int))
    ...
    def color_for_argb(a, r, g, b):
        if (0 <= a <= MAX_COMPONENT and 0 <= r <= MAX_COMPONENT and 0 <= g <= MAX_COMPONENT and 0 <= b <= MAX_COMPONENT):
            color = 0
            color |= (((a & MAX_COMPONENT) << 24) | ((r & MAX_COMPONENT) << 16) | ((g & MAX_COMPONENT) << 8) | (b & MAX_COMPONENT))
            return color
        return Error("invalid argb {}, {}, {}, {}".format(a, r, g, b))
...
def create_array(width, height, background=None):
    if np is not None:
        if background is None:
            ...
            return np.fromiter(iterable, np.uint32)
    else:
        typecode = "I" if array.array("I").itemsize >= 4 else "L"
        background = (background if background is not None else ColorForName["transparent"])
        return array.array(typecode, [background] * width * height)
    
"""
rgb.txtから取得され、ARGBに変換される(透明が追加される)
デフォルトは黒一色
"""
