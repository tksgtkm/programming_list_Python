"""
Abstract Factory
他のオブジェクトから構成された複雑なオブジェクトを生成したいとき、
そして、その構成オブジェクトからある特定のファミリーに属する場合に用いられる

ここでは古典的なAbstract Factory
簡単な図形を作成するプログラムを考える。
ここでは２つのファクトリーを使う。
・プレーンテキストを生成
・SVGを生成
...
def main():
    ...
    # プレーンテキストのファクトリーを使って図形を生成し、その図形を保存する
    txtDiagram = create_diagram(DiagramFactory())
    txtDiagram.save(textFilename)
    print("wrote", textFilename)

    # SVGファクトリーを使用して同じ図形を生成し保存する
...
def create_diagram(factory):
    """
    引数として図形ファクトリーを受け取り、それを使って図形を生成する。
...
# 一つのクラスをインターフェースを提供する基底クラスと具象クラスの両方の
# 機能を担うクラスとして用いるのが普通
